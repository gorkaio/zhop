defmodule Zhop.Catalog.ItemTest do
  ...
  test "it allows hyphens and underscores in item ids" do
    assert {:ok, %Item{id: "__T-E-S-T__", name: "My test", type: :product, price: Money.new(12_00)}} == Item.new("__T-E-S-T__", "My test", :product, Money.new(12_00))   
  end
  ...
  test "it allows values greater then zero for product prices" do
    ptest amount: int(min: 1) do
      assert {:ok, %Item{id: "TEST", name: "My test", type: :product, price: Money.new(amount)}} == Item.new("TEST", "My test", :product, Money.new(amount))   
    end
  end
  ...
  test "it allows values lesser than zero for discount prices" do
    ptest amount: int(max: -1) do      
      assert {:ok, %Item{id: "TEST", name: "My test", type: :discount, price: Money.new(amount)}} == Item.new("TEST", "My test", :discount, Money.new(amount))   
    end
  end
  ...
  test "it creates products" do
    ptest id: string(min: 1, max: 32, chars: :upper), name: string(min: 1, must_have: ["a"]), amount: positive_int() do
      assert {:ok, %Item{id: id, name: name, type: :product, price: Money.new(amount)}} == Item.new(id, name, :product, Money.new(amount))
    end
  end

  test "it creates discounts" do
    ptest id: string(min: 1, max: 32, chars: :upper), name: string(min: 1, must_have: ["a"]), amount: negative_int() do
      assert {:ok, %Item{id: id, name: name, type: :discount, price: Money.new(amount)}} == Item.new(id, name, :discount, Money.new(amount)) 
    end
  end
