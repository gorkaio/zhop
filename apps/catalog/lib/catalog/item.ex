defmodule Zhop.Catalog.Item do
  ...
  defguardp valid_item(id, name, type)
            when is_binary(id) and is_binary(name) and type in @valid_types
  ...
  """
  @spec new(id :: String.t(), name :: String.t(), type :: types(), money :: Money.t()) ::
          {:ok, Item.t()} | {:error, reason :: term}
  def new(id, name, type, %Money{} = price) when valid_item(id, name, type) do
    with {:ok, id} <- validate_id(id, type),
         ...
         {:ok, price} <- validate_price(price, type) do
